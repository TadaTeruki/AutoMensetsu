...
function update(text, ctx) {
  ...
  var ftime = time / Math.pow(10, timedigit);
  ctx.fillText(ftime.toFixed(timedigit), canvas_src.width / 2, canvas_src.height / 2, canvas_src.width);
}

function addtime(is_start,key, ctx) {
  if (is_start) {
    time = 0;
  } else {
    if (phase != 1) return;
  ...
  setTimeout(function() {
    addtime(false, key, ctx);
    update(q_sheet[vq_sheet[key]][1], ctx);
  }, 1000/Math.pow(10, timedigit));
